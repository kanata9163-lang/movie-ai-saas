...
import { runNarrationGeneration } from '@/lib/video/pipeline/orchestrator';
...
export async function POST(req: NextRequest, { params }: { params: { workspaceSlug: string; videoProjectId: string } }) {
  ...
  const creditError = await checkAndDeductCredits(auth.workspace.id as string, 'NARRATION', 'ナレーション生成', auth.userEmail);
  if (creditError) return creditError;

  const result = await runNarrationGeneration(params.videoProjectId);
